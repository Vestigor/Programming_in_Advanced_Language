...
public:
	...
	{
		double S;
		if ((p2.gety() - p1.gety()) * (p3.getx() - p1.getx()) - (p3.gety() - p1.gety()) * (p2.getx() - p1.getx()) == 0)
			return -1;
		S = fabs((p1.getx() * p2.gety() - p2.getx() * p1.gety()) + (p2.getx() * p3.gety() - p3.getx() * p2.gety()) + (p3.getx() * p1.gety() - p1.getx() * p3.gety())) / 2.0;
		return S;
	}
	/* 构造函数 */
